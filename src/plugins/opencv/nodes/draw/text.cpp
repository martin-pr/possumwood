...
dependency_graph::State compute(dependency_graph::Values& data) {
	...
	cv::putText(result, data.get(a_text), cv::Point(data.get(a_pos)[0], data.get(a_pos)[1]),
	            data.get(a_font).intValue(), data.get(a_fontScale),
	            cv::Scalar(data.get(a_color)[0], data.get(a_color)[1], data.get(a_color)[2]), data.get(a_thickness));
