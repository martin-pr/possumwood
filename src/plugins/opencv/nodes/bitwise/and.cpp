...
dependency_graph::State compute(dependency_graph::Values& data) {
	...
	if(data.get(a_mask).empty())
		cv::bitwise_and(*data.get(a_in1), *data.get(a_in2), mat);
	else
		cv::bitwise_and(*data.get(a_in1), *data.get(a_in2), mat, *data.get(a_mask));
...
possumwood::NodeImplementation s_impl("opencv/bitwise/and", init);
