...
dependency_graph::InAttr<float> a_sigma2; // LETS MAKE THIS SIGMA, NOT SIGMA^2
...
dependency_graph::State compute(dependency_graph::Values& data) {
	...
	// correspondence
	auto corresp = lightfields::gaussian::correspondence(data.get(a_samples), *data.get(a_in), tmp, data.get(a_sigma2));
...
void init(possumwood::Metadata& meta) {
	...
	meta.addInfluence(a_sigma2, a_correspondence);
