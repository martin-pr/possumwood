...
CameraIntrinsics::CameraIntrinsics(const cv::Mat& matrix, const std::vector<float>& distCoeffs,
                                   const cv::Size& imageSize)
    : m_matrix(matrix), m_distCoeffs(distCoeffs), m_imageSize(imageSize) {
}
...
bool CameraIntrinsics::operator!=(const CameraIntrinsics& f) const {
	return !sameMatrix(m_matrix, f.m_matrix) || !sameVector(m_distCoeffs, f.m_distCoeffs) ||
	       m_imageSize != f.m_imageSize;
}
