...
double BSpline<DEGREE>::B(double t, unsigned k) {
	...
	switch(k) {
		case 0: t = 1.0 - t; return t*t*t / 6.0;
...
template<unsigned DEGREE>
BSpline<DEGREE>::BSpline(unsigned subdiv, const std::array<double, DEGREE>& min, const std::array<double, DEGREE>& max) : m_subdiv(subdiv), m_controls(std::pow(subdiv+3, DEGREE), std::make_pair(0.0f, 0.0f)), m_min(min), m_max(max) {
	assert(subdiv > 0);
}
...
void BSpline<DEGREE>::visit(const std::array<double, DEGREE>& _coords, const FN& fn) const {
	...
	for(unsigned d=0; d<DEGREE; ++d) {
		...
		if(coords[d] < 0.0 || coords[d] > 1.0) {
			std::stringstream ss;
			ss << "Coordinate #" << d << " out of range - " << _coords[d] << " (expected between " << m_min[d] << " and " << m_max[d] << ")";
