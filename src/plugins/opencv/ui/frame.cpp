...
OIIO::TypeDesc::BASETYPE type(int cvType) {
	switch(cvType) {
		...
		default:
			throw std::runtime_error("Unknown OpenCV type passed to OIIO");
	}
...
FrameUI::FrameUI() {
	...
	QObject::connect(saveButton, &QToolButton::pressed, [this]() -> void {
		...
		if(!qfilename.isEmpty()) {
			...
			if(!out) {
				...
				err << "Could not create an ImageOutput for " << qfilename.toStdString()
				    << ", error = " << OIIO::geterror();
