...
MotionGraph::Snippet::Snippet(std::shared_ptr<const Animation> animation, std::size_t startFrame, std::size_t endFrame) : m_animation(animation), m_startFrame(startFrame), m_endFrame(endFrame) {
...
void MotionGraph::Snippet::appendTo(anim::Animation& anim) const {
	// relative transformation, to allow "integrating" the overall root movement, while discounting any initial offset of frames of the snippet.
	// For this to not accumulate errors, the root transformation has to represent "idealised trajectory". TODO: preprocessing of trajectories.
...
bool MotionGraph::Snippet::operator ==(const Snippet& snippet) const {
	return m_animation == snippet.m_animation && m_startFrame == snippet.m_startFrame && m_endFrame == snippet.m_endFrame && m_transitions == snippet.m_transitions;
}
...
MotionGraph::MotionGraph(const anim::Animation& a, const std::vector<std::pair<std::size_t, std::size_t>>& transitions) : m_fps(a.fps()) {
...
anim::Animation MotionGraph::randomWalk(std::size_t targetFrameCount, float transitionProbability, std::size_t seed) const {
