...
dependency_graph::State compute(dependency_graph::Values& data) {
	const std::shared_ptr<const possumwood::CGALPolyhedron> mesh = data.get(a_inMesh);
	...
	if(mesh) {
		std::unique_ptr<possumwood::CGALPolyhedron> newMesh(new possumwood::CGALPolyhedron(*mesh));
		...
		data.set(a_outMesh, std::shared_ptr<const possumwood::CGALPolyhedron>(newMesh.release()));
	}
	else
