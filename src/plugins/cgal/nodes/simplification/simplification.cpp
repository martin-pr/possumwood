...
dependency_graph::State compute(dependency_graph::Values& data) {
	...
	const unsigned algorithmId = data.get(a_stopCondition).intValue() +
	                             data.get(a_cost).intValue() +
	                             data.get(a_placement).intValue();
		...
		switch(algorithmId) {
			case 111:
				SMS::edge_collapse(
				    ...
				    CGAL::parameters::get_cost(
				        SMS::Edge_length_cost<possumwood::CGALPolyhedron>())
				        .get_placement(
				            SMS::Midpoint_placement<possumwood::CGALPolyhedron>())
				        .vertex_index_map(
				            get(CGAL::vertex_external_index, mesh.polyhedron()))
				        .halfedge_index_map(
				            get(CGAL::halfedge_external_index, mesh.polyhedron())));
				break;

			case 112:
				SMS::edge_collapse(
				    mesh.polyhedron(),
				    SMS::Count_ratio_stop_predicate<possumwood::CGALPolyhedron>(
				        stopCondition),
				    CGAL::parameters::get_cost(
				        SMS::Edge_length_cost<possumwood::CGALPolyhedron>())
				        .get_placement(
			...
			case 113:
				SMS::edge_collapse(
				    mesh.polyhedron(),
				    SMS::Edge_length_stop_predicate<float>(stopCondition),
				    CGAL::parameters::get_cost(
				        SMS::Edge_length_cost<possumwood::CGALPolyhedron>())
				        .get_placement(
				            SMS::Midpoint_placement<possumwood::CGALPolyhedron>())
			...
			case 121:
				SMS::edge_collapse(
				    ...
				    SMS::Count_stop_predicate<possumwood::CGALPolyhedron>(stopCondition),
				    CGAL::parameters::get_cost(
				        SMS::LindstromTurk_cost<possumwood::CGALPolyhedron>())
				        .get_placement(
				            SMS::Midpoint_placement<possumwood::CGALPolyhedron>())
				        .vertex_index_map(
				            get(CGAL::vertex_external_index, mesh.polyhedron()))
			...
			case 211:
				SMS::edge_collapse(
				    ...
				    CGAL::parameters::get_cost(
				        SMS::Edge_length_cost<possumwood::CGALPolyhedron>())
				        .get_placement(
				            SMS::LindstromTurk_placement<possumwood::CGALPolyhedron>())
				        .vertex_index_map(
				            get(CGAL::vertex_external_index, mesh.polyhedron()))
				        .halfedge_index_map(
				            get(CGAL::halfedge_external_index, mesh.polyhedron())));
...
void init(possumwood::Metadata& meta) {
	...
	meta.addAttribute(
	    ...
	    possumwood::Enum({std::make_pair("Count_stop_predicate", 1),
	                      std::make_pair("Count_ratio_stop_predicate", 2),
	                      std::make_pair("Edge_length_stop_predicate", 3)}));
	meta.addAttribute(a_cost, "cost",
	                  possumwood::Enum({std::make_pair("Edge_length_cost", 10),
	                                    std::make_pair("LindstromTurk_cost", 20)}));
	meta.addAttribute(a_placement, "placement",
