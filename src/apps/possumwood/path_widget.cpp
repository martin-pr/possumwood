...
PathWidget::PathWidget(QWidget* parent) : QWidget(parent) {
	...
	button->connect(button, &QToolButton::pressed, [this]() { emitChangeNetwork(0); });
...
void PathWidget::goToPath(const Path& path) {
	...
	while(m_layout->count() < (int)path.m_path.size()) {
		...
		connect(button, &QToolButton::pressed, [this, id]() { emitChangeNetwork(id); });
	...
	for(int i = 0; i < (int)path.m_path.size(); ++i) {
		...
		if(i == 0)
			...
			auto it =
			    possumwood::App::instance().graph().nodes().find(path.m_path[i], dependency_graph::Nodes::kRecursive);
