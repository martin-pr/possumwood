...
const boost::filesystem::path Filename::filename(bool makeAbsolute) const {
	if(!makeAbsolute)
		...
		return m_filename.toPath();
}

void Filename::setFilename(const boost::filesystem::path& filename) {
	m_filename = possumwood::Filepath::fromPath(filename);
}
...
void toJson(::nlohmann::json& json, const Filename& value) {
	json = value.filename(false).string();
}

void fromJson(const ::nlohmann::json& json, Filename& value) {
