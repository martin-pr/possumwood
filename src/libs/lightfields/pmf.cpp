...
PMF PMF::combine(const PMF& p1, float w1, const PMF& p2, float w2) {
	// this should be implemented as a convolution. However, we'd need to normalize the result afterwards anyway, which
	// will lead to simple sum no matter what.
...
JointPMF::JointPMF(unsigned rows, unsigned cols) : m_rows(rows), m_cols(cols) {
	...
	m_fn = [invsize](unsigned row, unsigned col) { return invsize; };
}
