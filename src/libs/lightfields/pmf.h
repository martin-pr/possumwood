...
class PMF {
  public:
	/// initialises a PMF from a confidence value (0..1; 0 -> no confidence, leading to a uniform distribution; 1 ->
	/// 100% confidence, leading to a single peak of p()=1)
...
/// In Possumwood, currently PMFs are represented procedurally using a functor (which might potentially hold explicit
/// values though).
class JointPMF {
  public:
