...
unsigned BFSVisitors::vec2index(const Index& v) const {
	...
	const unsigned index = unsigned(v.pos.x - m_range.cols().begin()) +
	                       unsigned(v.pos.y - m_range.rows().begin()) * m_range.cols().size() + v.n_layer * m_layerSize;
...
Index BFSVisitors::index2vec(unsigned i) const {
	...
	return Index{
	    V2i(ii % m_range.cols().size() + m_range.cols().begin(), ii / m_range.cols().size() + m_range.rows().begin()),
	    layer};
}

BFSVisitors::BFSVisitors(const tbb::blocked_range2d<int>& range, unsigned layerCount)
    : m_range(range), m_layerCount(layerCount), m_layerSize(m_range.rows().size() * m_range.cols().size()),
