...
void Graph::setLayer(const cv::Mat& vals, const std::size_t& index, float power) {
	...
	for(int y = 0; y < vals.rows; ++y)
		for(int x = 0; x < vals.cols - 1; ++x) {
			const int cap = powi(255 - differential(vals.ptr<unsigned char>(y, x), vals.ptr<unsigned char>(y, x + 1),
			                                        vals.channels()),
			                     power) *
			                m_nLinkValue / 256;
	...
	for(int y = 0; y < vals.rows - 1; ++y)
		for(int x = 0; x < vals.cols; ++x) {
			const int cap = powi(255 - differential(vals.ptr<unsigned char>(y, x), vals.ptr<unsigned char>(y + 1, x),
			                                        vals.channels()),
			                     power) *
			                m_nLinkValue / 256;
			...
			while(!stack.empty()) {
				...
				if(!visited[v2i(current.pos) + current.n_layer * m_size.x * m_size.y]) {
					...
					if(current.n_layer < m_nLinks.size()) {
						// horizontal move
						if(current.pos.x > 0 && m_nLinks[current.n_layer]
						                                .edge(current.pos, V2i(current.pos.x - 1, current.pos.y))
						                                .residualCapacity() > 0)
							stack.push_back(Index{V2i(current.pos.x - 1, current.pos.y), current.n_layer});
						if(current.pos.x < m_size.x - 1 &&
						   m_nLinks[current.n_layer]
						           .edge(current.pos, V2i(current.pos.x + 1, current.pos.y))
						           .residualCapacity() > 0)
							stack.push_back(Index{V2i(current.pos.x + 1, current.pos.y), current.n_layer});

						// vertical move
						if(current.pos.y > 0 && m_nLinks[current.n_layer]
						                                .edge(current.pos, V2i(current.pos.x, current.pos.y - 1))
						                                .residualCapacity() > 0)
						...
						if(current.n_layer > 0 &&
						   m_tLinks[current.n_layer].edge(current.pos).backward().residualCapacity() > 0)
							stack.push_back(Index{current.pos, current.n_layer - 1});
					}
				}
			}
