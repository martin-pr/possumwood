...
void doConnectByNames(const dependency_graph::UniqueId& fromNode, const std::string& fromPort,
	...
	if(fromPortId < 0)
		throw std::runtime_error("Cannot connect " + from.name() + ":" + fromPort +
...
void doDisconnectByNames(const dependency_graph::UniqueId& fromNode, const std::string& fromPort,
                         const dependency_graph::UniqueId& toNode, const std::string& toPort) {
	dependency_graph::NodeBase& from = detail::findNode(fromNode);
	dependency_graph::NodeBase& to = detail::findNode(toNode);
...
possumwood::UndoStack::Action disconnectAction(dependency_graph::Port& p1, dependency_graph::Port& p2) {
	return disconnectAction(p1.node().index(), p1.index(), p2.node().index(), p2.index());
}
...
possumwood::UndoStack::Action connectAction(const dependency_graph::UniqueId& fromNodeId, std::size_t fromPort,
	...
	{
		std::stringstream ss;
		ss << "Cloning data for a new connection between " << fromNodeId << "/" << fromPort << " and " << toNodeId
		   << "/" << toPort;
...

	{
		...
		action.addCommand(ss.str(), std::bind(&doConnectByIds, fromNodeId, fromPort, toNodeId, toPort),
		                  std::bind(&doDisconnectByIds, fromNodeId, fromPort, toNodeId, toPort));
	}
...
possumwood::UndoStack::Action connectAction(const dependency_graph::UniqueId& fromNodeId,
	...
	{
		std::stringstream ss;
		ss << "Cloning data for a new connection between " << fromNodeId << "/" << fromPortName << " and " << toNodeId
		   << "/" << toPortName;
